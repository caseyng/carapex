...
**ScriptResult** — the return type of `ScriptChecker.inspect()`. A `SafetyResult` subclass that always carries `safe=true` and additionally carries `detected_language` (string or null) and `translation_needed` (boolean). Passed to `Translator.set_prior_result()` before translation is attempted. Internal to the pipeline — not part of `EvaluationResult`.
...
| `input_safety_check` | `safe` (boolean), `failure_mode` (string), `reason` (string) — `failure_mode` and `reason` absent if `safe=true` |
...
| `output_safety_check` | `safe` (boolean), `failure_mode` (string), `reason` (string) — `failure_mode` and `reason` absent if `safe=true` |
...
**What Translator receives:** A `ScriptResult` — a `SafetyResult` subclass carrying `detected_language` and `translation_needed`. Translator reads `translation_needed` directly. It does not re-detect the language.
...
| Field | Type | Default | Meaning |
|---|---|---|---|
| `injection_patterns` | list of strings or null | null | Regex patterns for the input PatternChecker. `null` raises `ConfigurationError` — see §8 for the full rationale. `[]` also raises `ConfigurationError`. Supply a non-empty list of patterns, or omit the field entirely to use the built-in default set. Custom patterns replace the built-in set entirely. |
